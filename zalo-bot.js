...
(async () => {
  ...
  const userDataDir = process.env.USER_DATA_DIR || '/data/chrome-profile'; // dùng Volume Railway

  const browser = await puppeteer.launch({
    headless: true, // chạy headless để deploy trên Railway
  ...
  await page.exposeFunction('onNewMessage', async msg => {
    ...
    if (msg.text && msg.text.startsWith('!')) {
      ...
      if (t === '!check') { 
        const n = s.current_submitters.length; 
        let r = `Đã có ${n} người:\n`; 
  ...
  await page.evaluate(() => {
    function extract(node) {
      ...
      let senderName = ''; 
      try { const s = node.querySelector('.name,.author-name'); if (s) senderName = s.innerText.trim(); } catch(e) {}
  ...
  async function sendGroupMessage(page, text) {
    await page.evaluate(t => {
      ...
      if (!input) { console.warn('Không tìm thấy ô chat'); return; }
      input.focus(); 
